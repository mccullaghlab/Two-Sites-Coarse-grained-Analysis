...
def iterative_align_average_2_site(coord,selGroup1,selGroup2,frameStart=0,frameStop=-1,deltaFrame=1,maxSteps=25,thresh=1.0E-10):
    ...
    alignedPos = np.empty((nFrames,(selGroup1.n_residues+selGroup2.n_residues),3),dtype=np.float64)
    #generate an initial average by aligning to first frame
    avg = np.zeros(((selGroup1.n_residues+selGroup2.n_residues),3),dtype=np.float64)
    comPos = np.empty(((selGroup1.n_residues+selGroup2.n_residues),3),dtype=np.float64)
    frameCount = 0
    for ts in coord.trajectory[frameStart:frameStop:deltaFrame]:
        ...
        for i, resid in enumerate(np.unique(selGroup1.resids)):
            ...
            comPos[count,:] = selGroup1.select_atoms(residSel).center_of_mass()
            count += 1
            for j, resid2 in enumerate(np.unique(selGroup2.resids)):
             if resid2 == resid:
                    residSel = "resid " + str(resid2)
                    comPos[count,:] = selGroup2.select_atoms(residSel).center_of_mass()
                    count += 1
        if frameCount == 0:
            ref = np.copy(comPos)
        else:
    ...
    newAvg = np.zeros(((selGroup1.n_residues+selGroup2.n_residues),3),dtype=np.float64)
    avgRmsd = 2*thresh
    step = 0
...
def iterative_align_average(coord,selGroup,frameStart=0,frameStop=-1,deltaFrame=1,maxSteps=25,thresh=1.0E-10):
    if frameStop < 0:
        frameStop = coord.trajectory.n_frames + frameStop + 1
    nFrames= int((frameStop-frameStart)//deltaFrame)
    # create numpy array of aligned positions
...
def perform_reach_traj(coord,sel,nWindows=1,deltaFrame=1,thresh=1.0E-10):
...
# sel is an MDAnalysis atom group
def perform_reach_traj_com(coord,sel,nWindows=1,deltaFrame=1,thresh=1.0E-10):
    # setup some variables for windows and trajectory
    totalFrames = coord.trajectory.n_frames
    windowFrames = totalFrames//nWindows
    # declare arrays
...
def iterative_align_average_com(coord,selGroup,frameStart=0,frameStop=-1,deltaFrame=1,maxSteps=25,thresh=1.0E-10):
    if frameStop < 0:
        frameStop = coord.trajectory.n_frames + frameStop + 1
    nFrames= int((frameStop-frameStart)//deltaFrame)
    # create numpy array of aligned positions
